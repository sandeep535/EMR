...
export default function BillsList({ clientId }) {
  ...
  async function getPendingBillsByClientId(clientId) {
    var payLoad = {
  ...
  return (
    <>
      <Box sx={{ flexGrow: 1, m: 1 }}>
        ...
        <CommonCard
          title={Translations.BILLS_LIST.PENDING_BILLS}
          iconsList={[{ icon: "credit_card" }]}
          catchCliedEvent={() => {
            clickgenerateBill();
          }}
        >
          <CustomDataGrid
            tableHeaders={pendingbillListHeaders}
            tableData={pendingbilltableData}
